  ...
  <body>
    ...
    <script>
      ...
      async function callTool(name, args) {
        // Prefer window.openai (M365 Copilot Skybridge)
        if (window.openai && typeof window.openai.callTool === "function") {
          return window.openai.callTool(name, args || {});
        }
        // Fallback: window.ai.tools.call (OAI Apps bridge)
      ...
      function extractWorker(response) {
        ...
        // structuredContent path (direct payload)
        if (response.structuredContent) {
          return response.structuredContent;
        }
        // Direct worker fields
        ...
        // toolOutput pre-populated (M365 Copilot pattern)
      ...
      // Auto-load: check for pre-populated toolOutput first
      if (window.openai && window.openai.toolOutput) {
